...
function createMarker(place) {
  ...
  google.maps.event.addListener(marker, 'click', function () {
    infowindow.setContent('<div><strong>Name:</strong> ' + place.name + '</div>' + '<div><strong>Address: </strong>' + place.vicinity + '</div>' + '<div><strong>Open Now:</strong> ' + place.opening_hours.open_now + '</div>' + '<div><strong>Rating:</strong> ' + place.rating + '</div>' + '<div>' + '<div><img src=' + photos[0].getUrl({
      'maxWidth': 150,
      'maxHeight': 150
    }) + '></div>');
